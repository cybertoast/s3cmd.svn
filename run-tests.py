...
if os.name == "posix":
    have_wget = True
    if os.uname() and os.uname()[0] == "Darwin":
        print("\x1b[31;1mWARNING: %s\x1b[0m" % "Keep in mind that some tests may fail because of case-sensitivity!")
elif os.name == "nt":
    have_wget = False
    print("Keep in mind that some tests may fail because of case-sensitivity!")
else:
...
## OSX, Windows are case-insensitive and that's messed up!
...
test_s3cmd("Sync between S3 buckets", ['sync', pbucket(1) + '/xyz/', pbucket(1) + '/abc/', '--exclude', 'demo/*', '--exclude', '*.png', '--no-encrypt', '--exclude-from', 'testsuite/exclude.encodings' ],
    must_find = [ "File %(pbucket)s/xyz/binary/random-crap copied to %(pbucket)s/abc/binary/random-crap" % { 'pbucket' : pbucket(1) },
                  "File %(pbucket)s/xyz/exclude.encodings copied to %(pbucket)s/abc/exclude.encodings"
                  % { 'pbucket' : pbucket(1) }],
    must_not_find_re = [ "demo/", "\.png$", "permission-denied-dir" ])

if have_encoding:
    ## ====== Sync UTF-8 / GBK / ... to S3
    test_s3cmd("Sync %s between S3 buckets" % encoding, ['sync', '%s/xyz/encodings/%s' % (pbucket(1), encoding), '%s/abc/encodings/' % pbucket(1), '--exclude', 'demo/*', '--no-encrypt' ],
        must_find = [ u"File '%(pbucket)s/xyz/encodings/%(encoding)s/%(pattern)s' stored as '%(pbucket)s/abc/encodings/%(encoding)s/%(pattern)s'" % { 'encoding' : encoding, 'pattern' : enc_pattern , 'pbucket' : pbucket(1)} ])
